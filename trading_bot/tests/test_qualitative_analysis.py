...
class TestQualitativeAnalysis(unittest.TestCase):
    ...
    @patch('trading_bot.src.analysis.qualitative_analysis.requests.get')
    def test_fetch_google_news(self, mock_get):
        mock_response = MagicMock()
        mock_response.json.return_value = {'articles': []}
        mock_get.return_value = mock_response

        qa = QualitativeAnalysis('fake_google_news_api_key', 'fake_twitter_api_key', 'fake_twitter_api_secret')
        result = qa.fetch_google_news('AAPL')
    ...
    @patch('trading_bot.src.analysis.qualitative_analysis.QualitativeAnalysis.fetch_google_news')
    @patch('trading_bot.src.analysis.qualitative_analysis.QualitativeAnalysis.fetch_twitter_data')
    def test_get_qualitative_analysis(self, mock_fetch_twitter_data, mock_fetch_google_news):
        mock_fetch_google_news.return_value = {'articles': [{'title': 'Great news', 'description': 'Stock is up!'}]}
